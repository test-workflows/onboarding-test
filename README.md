...
## Running testing in Quarkus dev mode
...
```
(testing should be included in the returned array)
...
[testing-schema.json](./src/main/resources/schemas/testing-schema.json ), run the following to create an
instance of the testing workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/testing -d@input.json
```
